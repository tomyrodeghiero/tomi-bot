...
def manage_chatbot(text, number, messageId, name):
    text = text.lower()  # message that the user sent
    messages = []
    ...
    if "hola" in text:
        ...
        messages.extend([replyReaction, replyButtonData])
        ...
        messages.extend([listReplyData, sticker])

    elif "inteligencia artificial para mi negocio" in text:
    ...
    elif "✅ sí, agenda una reunión" in text or "📅 agendar cita" in text:
